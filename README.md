...
Public reference for the **Moxie Docs** Model Context Protocol (MCP) server and the agent **skills** Moxie installs into your repository.
...
1. **Connect a repository** at https://moxiedocs.com — install the GitHub App and let Moxie index the repo.
2. **Create an MCP token** in the dashboard (Settings → MCP). Tokens are organization-wide; keep them secret.
3. **Add the server to your agent** using a config block from [Client configuration](#client-configuration).
4. **Let your agent read context first.** Point it at the [`moxie-docs` skill](#the-moxie-docs-skill) (Moxie also opens a PR that installs it for you), then it calls `moxie.get_ai_context` before editing and proposes doc updates in the same PR.
